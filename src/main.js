...
async function main() {
  try {
    ...
    console.log(`⏱️ Tiempo total de ejecución: ${(results.stats.totalDuration / 1000).toFixed(2)} segundos`);
    ...
    Object.entries(results.stats.jobs.byCompany)
    ...
    if (results.stats.jobs.byJobType) {
      ...
      Object.entries(results.stats.jobs.byJobType)
        .sort(([,a], [,b]) => b - a)
    ...
    if (results.stats.jobs.byLocation) {
      ...
      Object.entries(results.stats.jobs.byLocation)
        .sort(([,a], [,b]) => b - a)
    ...
    console.log(`📁 Resultados consolidados guardados en: ${pipeline.config.outputDir}/${pipeline.config.consolidatedFile}`);
    console.log(`📊 Estadísticas detalladas en: ${pipeline.config.outputDir}/${pipeline.config.statsFile}`);

    // Sugerencias para el usuario
    console.log('\n💡 SUGERENCIAS:');
    console.log('   • Para ejecutar solo algunos scrapers, deshabilita los otros cambiando enabled: false');
    ...
    console.log('   • Puedes cambiar parallel: true para scraping más rápido (cuidado con rate limits)');
